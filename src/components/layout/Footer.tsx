...
const ADDRESS_EN = 'Al Thawra Street 107, Cairo, Cairo, Egypt.';
const ADDRESS_AR = 'شارع الثورة ١٠٧، القاهرة، مصر.';
...
const Footer = () => {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') {
          toast.info(isAr ? 'أنت مشترك بالفعل!' : 'You are already subscribed!');
        } else {
          throw error;
        }
      } else {
        toast.success(isAr ? 'تم الاشتراك بنجاح في نشرتنا الإخبارية!' : 'Successfully subscribed to our newsletter!');
        setEmail('');
      }
    } catch {
  ...
  const quickLinks = [
    ...
    { href: '/find-property', label: t('footer.findProperty') },
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3 text-muted-foreground">
                ...
                <span>{isAr ? ADDRESS_AR : ADDRESS_EN}</span>
              </li>
